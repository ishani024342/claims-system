      ...
      <div style={styles.main}>
        ...
        {section === 'dashboard' && (
          <div style={styles.statsGrid}>
            {[
              { icon: '👥', label: 'Policyholders', value: stats.policyholders, color: '#4f46e5' },
              { icon: '📋', label: 'Policies', value: stats.policies, color: '#0891b2' },
              { icon: '📝', label: 'Total Claims', value: stats.claims, color: '#7c3aed' },
              { icon: '✅', label: 'Approved Claims', value: stats.approved, color: '#059669' },
            ].map(s => (
              <div key={s.label} style={styles.statCard}>
                <div style={{ ...styles.statIconBox, background: s.color + '15' }}>
          ...
          <div>
            <div style={styles.card}>
              ...
              <form onSubmit={submitPh} style={styles.formGrid}>
                ...
                {editingPh && <button type="button" onClick={() => { setEditingPh(null); setPhForm({ id: '', name: '', email: '', phone: '' }); }} style={{ background: '#e5e7eb', color: '#333' }}>Cancel</button>}
              </form>
            </div>
...
const styles = {
  ...
  logo: { fontSize: '18px', fontWeight: '700', padding: '8px 12px', marginBottom: '24px', color: '#a78bfa' },
  nav: { display: 'flex', flexDirection: 'column', gap: '4px', flex: 1 },
  navBtn: {
    background: 'transparent', color: '#ccc', padding: '10px 12px',
    borderRadius: '8px', textAlign: 'left', fontSize: '14px', border: 'none', cursor: 'pointer',
  },
